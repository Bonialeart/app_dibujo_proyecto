...
public:
    ...
    void composite(const ImageBuffer& other, int offsetX = 0, int offsetY = 0, float opacity = 1.0f);
    ...
    static std::unique_ptr<ImageBuffer> fromBytes(const std::vector<uint8_t>& bytes, int width, int height);
